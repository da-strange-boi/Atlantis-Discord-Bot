...
exports.run = async (bot) => {
  ...
  app.get('/', async (req, res) => {
    res.render('index', {
      ...
      helpers: {
        ...
        getUptime: function () { const time = ms(bot.uptime); return `${time.days} days, ${time.hours}h ${time.minutes}m ${time.seconds}s ${time.milliseconds}ms` }
      }
    })
  })
  ...
  app.get('/members', async (req, res) => {
    const aotwMembers = await bot.database.Website.find({}).sort({ owosPastWeek: -1 }).limit(3).toArray()
    const aotwID = []
    for (let i = 0; i < aotwMembers.length; i++) { aotwID.push(aotwMembers[i].userID) }
    const members = await bot.database.Website.find({ userID: { $nin: aotwID } }).sort({ owosPastWeek: -1 }).toArray()
  ...
  app.get('/member/:userID', async (req, res) => {
    res.render('member', {
      ...
      helpers: {
        ...
        owoCountTime: function (obj) { const time = ms((obj * 10) * 1000); return `${time.days}d ${time.hours}h ${time.minutes}m ${time.seconds}s` }
      }
    })
  })
  ...
  app.get('/weekReset', async (req, res) => {
    ...
    await bot.database.Website.find({}).forEach(async (member) => {
      ...
      const userAchieves = member.owosArchive
