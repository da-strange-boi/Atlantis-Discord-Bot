...
exports.run = async (bot) => {
  bot.registerCommand("deletebotmessages", async (message, args) => {
      ...
      if (!args[0]) {
        const helpEmbed = {
          embed: {
            ...
            description: "***deletebotmessages*** will delete all bot messages in a given channel\n\n__**add**__ ~ Add a channel to the category\nexample: `a!deletebotmessages add #channel`\n\n__**delete**__ ~ Deletes a channel from the category\nexample: `a!deletebotmessages delete #channel`",
            timestamp: new Date()
          }
        }
